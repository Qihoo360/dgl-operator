...
def reviseIPConfig(ip_config):
    local_ip_config = os.path.dirname(ip_config) + '/hostfile_revised'
    tuples = []
    ...
    with open(local_ip_config, "w") as f:
        for el in tuples:
            li = el.split()
            f.write(f'{li[0]} {li[1]}\n')
    os.environ['DGL_IP_CONFIG'] = local_ip_config
    return local_ip_config
...
def main(args):
    args.ip_config = reviseIPConfig(args.ip_config)
