...
class ArgParser(CommonArgParser):
    def __init__(self):
        ...
        self.add_argument('--host_config', type=str, help='Host configuration file.')
        self.add_argument('--num_client_proc', type=int, default=1,
                          help='Number of client process on each machine.')
        self.add_argument('--num_servers', type=int, default=1, help='The number of servers')
...
def kube_exec(cmd_str, host):
    kubexec_path = os.getenv('DGL_OPERATOR_KUBEXEC_PATH')
    cmd_str = '%s %s \"%s\"' % (kubexec_path, host, cmd_str)
    return cmd_str

def kube_cp(file, host, path):
    kubectl_path = os.getenv('DGL_OPERATOR_KUBECTL_PATH')
    scp_cmd_str = '%s cp %s %s:%s' % (kubectl_path, file, host, path)
    return scp_cmd_str
...
def launch(args):
    ...
    """
    use_kube = False
    if 'DGL_OPERATOR_ENV' in os.environ and os.getenv('DGL_OPERATOR_ENV') == '1':
        use_kube = True
    ...
    if use_kube:
        with open(args.host_config) as f:
            for line in f:
                _, _, host, _ = line.strip().split(' ')
                cmd_str = kube_cp(file_path, host, args.path)
                print('Pod %s cmd: %s' % (host, cmd_str))
    ...
    if use_kube:
        with open(args.host_config) as f:
            ...
            for line in f:
                ...
                cmd_str = '%s/%s %d %d' % (args.path, SCRIPT_FILE, id_low, id_high)
                cmd_str = kube_exec(cmd_str, host)
                print('Pod %s cmd: %s' % (host, cmd_str))
                job_list.append(run_cmd(cmd_str))
                cmd_list.append(cmd_str)
                machine_id += 1
